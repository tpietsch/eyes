...
public class UserTweetRest {
    ...
    @ResponseBody
    @Transactional
    @PreAuthorize("(@tweetSecurity.isUserTweetForCurrentUser(authentication,#userId) and @tweetSecurity.isUserTweetForCurrentUser(authentication,#tweetEntity)) or hasRole('ROLE_ADMIN')")
